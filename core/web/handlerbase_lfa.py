...
class HandlerBase(RequestHandler):
    ...
    def initialize(self):
        ...
        current_user_permissions = current_user and "permissions" in current_user and  current_user['permissions'] or []
        ...
        self.context.UP = current_user_permissions
    ...
    def redis_cache(self, key, value):
        '''
            ...
            隆福安的特殊处理: 5836a1e31d41c80c57fce618 由管理员的权限转为代理商
        '''
        result = None
        ...
        if key == "user":
            if not result:
                ...
                if result:
                    ...
                    permissions = []
                    if value == '5836a1e31d41c80c57fce618':
                        ps = db.role.find_one({"roleid": "agent"}).permission
                    else:
                        ps = result.userrole.fetch().permission
                    for pid in ps:
                    ...
                    self.cache.set(redis_key, doc_result)
            else:
            ...
            if not result:
                ...
                if result:
                    ...
                    self.cache.set(redis_key, result)
            else:
                result = ObjectDict(eval(result))
            ...
            if result:
                ...
                for pid in result.userrole.fetch().permission:
                    ...
                    if p:
                        permissions.append(p.code)
                result["permissions"] = permissions
                self.cache.set(redis_key, ObjectDict(result))
                return True
        if key == "systemconfig":
            result = db.systemconfig.find_one({ "key" : value })
            if result:
