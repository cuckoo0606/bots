...
try:
    from settings import WECHAT_SINGN_KEY, WECHAT_PAY_APPID, WECHAT_PAY_MCHID, WECHAT_PAY_CREATE_IP, WECHAT_APP_SECRET
except:
    WECHAT_SINGN_KEY = ""
    WECHAT_PAY_APPID = ""
    WECHAT_PAY_MCHID = ""
    WECHAT_PAY_CREATE_IP = ""
    WECHAT_APP_SECRET = ""
...
WECHAT_JSAPI_TOKEN_URL = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid={0}&secret={1}"
WECHAT_JSAPI_TICKET_URL = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token={0}&type=jsapi"


class WeChatPay(object):
    def get_auth_url(self, url, state):
        """
            url访问成功后会返回code，用于获取微信用户资料
        """
        return WECHAT_AUTHORIZE_URL.format(WECHAT_PAY_APPID, url, state)
    ...
    def get_openid(self, code):
        ...
        """
        url = WECHAT_AUTH_TOKEN_URL.format(WECHAT_PAY_APPID, WECHAT_APP_SECRET, code)
    ...
    def get_jsapi_ticket(self):
        ...
        """
        url = WECHAT_JSAPI_TOKEN_URL.format(WECHAT_PAY_APPID, WECHAT_APP_SECRET)
        response = requests.get(url)
        result = response.json()
        
        url = WECHAT_JSAPI_TICKET_URL.format(result["access_token"])
        response = requests.get(url)
        return response.json()
    ...
    def get_user_info(self, code):
        ...
        """
        result = self.get_openid(code)

        url = WECHAT_USERINFO_URL.format(result["access_token"], result["openid"])
        response = requests.get(url)
        return response.json()
...
def jsapi_sign(request):
    """
        生成jsapi加密签名
    """
    f = [ "jsapi_ticket", "noncestr", "timestamp", "url" ]
...
if __name__ == "__main__":
    ...
    request.body = "移动流量包100M"
    request.detail = "测试"
    request.attach = "和沃易"
    request.out_trade_no = str(uuid.uuid1()).replace("-", "")
