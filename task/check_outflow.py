...
def show_infos():
    ...
    outflows = db.outflow.find( {"status": 1, "created" : {"$gt" : start, "$lt" : end}} )
    ...
    hengyi_0214.writerow([ '订单号'.decode('utf8').encode('gbk'), '用户id'.decode('utf8').encode('gbk'), '用户名称'.decode('utf8').encode('gbk'), \
        ...
        '银行账号'.decode('utf8').encode('gbk'), '开户人'.decode('utf8').encode('gbk'), '时间'.decode('utf8').encode('gbk') ])
    ...
    for i in outflows:
        ...
        hengyi_0214.writerow([ i.no, userid, username.decode('utf8').encode('gbk'), amount, handling_charge, bank.decode('utf8').encode('gbk'), \
            bankbranch.decode('utf8').encode('gbk'), "\t"+bankaccount, bankholder.decode('utf8').encode('gbk'), created ])
